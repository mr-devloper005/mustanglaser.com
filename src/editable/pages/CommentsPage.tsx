...
const COMMENT_KEY_PREFIX = 'editable:article-comments:'
  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-6 shadow-[0_22px_60px_rgba(94,0,6,0.08)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="editable-display mt-4 text-5xl font-semibold tracking-[-0.06em] sm:text-6xl">Comments</h1>
            ...
            <button type="button" className="rounded-full border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] px-4 py-3 text-xs font-semibold uppercase tracking-[0.16em]" onClick={refreshComments}>Refresh comments</button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="editable-input pl-9 pr-3 text-sm"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[1.7rem] border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-5 shadow-[0_16px_42px_rgba(94,0,6,0.06)]">
          ...
          <section className="mt-8 rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-8 text-center">
            <h2 className="editable-display text-2xl font-semibold text-[var(--slot4-page-text)]">No comments yet</h2>
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[1.7rem] border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-4 text-sm text-[var(--slot4-muted-text)]">
            <span>Page {currentPage} of {totalPages}</span>
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-[var(--editable-border)] px-4 py-3 text-xs font-semibold uppercase tracking-[0.16em] disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
              <button type="button" className="rounded-full border border-[var(--editable-border)] px-4 py-3 text-xs font-semibold uppercase tracking-[0.16em] disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
            </div>
          </div>
        ) : null}
      </main>
