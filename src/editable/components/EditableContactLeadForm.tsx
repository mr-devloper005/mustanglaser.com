...
export function EditableContactLeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 rounded-[1.8rem] border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-6 shadow-[0_18px_46px_rgba(94,0,6,0.06)] md:p-8">
      ...
      <label className="mt-4 grid gap-2 text-sm font-semibold text-[var(--slot4-muted-text)]">
        ...
        <textarea
          ...
          className="editable-input min-h-36 py-3 text-base font-medium"
        />
      </label>
      <input name="company" tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true" />
      {message ? (
        <div className={`mt-5 flex items-start gap-3 rounded-[1rem] px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-emerald-950/40 text-emerald-300' : 'bg-red-950/40 text-red-300'}`}>
  ...
  return (
    <label className="grid gap-2 text-sm font-semibold text-[var(--slot4-muted-text)]">
      ...
      <input
        ...
        className="editable-input text-base font-medium"
      />
    </label>
  )
}
