...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  const persist = (next: Comment[]) => {
    ...
    try {
      ...
      // Storage can fail in private modes; keep the in-memory list instead.
    }
  }
  ...
  return (
    <section className="mt-14 border-t border-[var(--tk-line)] pt-10">
      ...
      <form onSubmit={submit} className="mt-6 rounded-[1.7rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 shadow-[0_16px_44px_rgba(94,0,6,0.06)]">
        <input
          value={name}
          onChange={(event) => setName(event.target.value)}
          placeholder="Your name (optional)"
          maxLength={60}
          className="editable-input text-sm"
        />
        <textarea
          value={text}
          ...
          rows={3}
          maxLength={1500}
          className="editable-input mt-3 min-h-28 resize-y py-3 text-sm leading-6"
        />
        <div className="mt-3 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 rounded-full bg-[var(--tk-accent)] px-6 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--tk-on-accent)] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div className="mt-6 grid gap-3">
        {all.map((comment) => (
          <div key={comment.id} className="rounded-[1.5rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 shadow-[0_12px_32px_rgba(94,0,6,0.05)]">
